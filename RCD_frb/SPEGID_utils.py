...
def get_benchmark_pulsars(survey='PALFA'):
    if survey == 'GBT':
        benchmark_data_full = GBT_benchmark_data_full
    elif survey == 'PALFA':
        benchmark_data_full = PALFA_benchmark_data_full
    ...
    for pulsar in pulsars:
        ...
        dimmest_SNR = df_pulsar['peak_SNR'].min()
        peak_DM = df_pulsar['centered_DM'].tolist()[0]
        cur_pulsar = benchmarkPulsar(pulsar, n_pulses, brightest_SNR, dimmest_SNR, peak_DM)
...
class benchmarkPulsar(object):
    ...
    __slots__ = ["beam", "n_pulses", "brightest_SNR", "dimmest_SNR","peak_DM"]

    def __init__(self, beam, n_pulses, brightest_SNR, dimmest_SNR, centered_DM):
        """
        ...
        self.dimmest_SNR = dimmest_SNR
        self.peak_DM = centered_DM
    ...
    def __str__(self):
        ...
        s = ["\tpulsar beam: \t\t%s" % self.beam,
             ...
             "\tdimmest SNR: \t\t%3.2f" % self.dimmest_SNR,
             "\tpeak DM: \t\t%3.2f" % self.peak_DM,
             "--------------------------------"
             ]
        return '\n'.join(s)
